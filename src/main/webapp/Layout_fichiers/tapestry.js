...
/*
 * Modifie par Atos Worldline
 * 
 * Modifications :
 * 		- Tapestry.ElementAdditions.isDeepVisible
 * 		- Tapestry.ErrorPopup (permet d'afficher correctement les messages d'erreur sous IE)
 * 		- Tapestry.ElementEffect (regle le rafraichissement parfois complique des zones et des formFragments)
 * 		- Tapestry.FormInjector (prend en compte les modifications de Tapestry.ElementEffect)
 */
...
Tapestry.ElementEffect = {
	...
	/**
	 * Modification des methodes show et fade.
	 * Objectif : avoir toujours l'affichage d'un form fragment.
	 * Pb : lors de 2 clics successifs pour effacer puis afficher un formFragment, celui-ci ne s'affiche pas toujours.
	 * Solution : Un semaphore bloque la fonction show jusqu'a l'arret complet de la methode fade.
	 */
    ...
    {	
    	...
    	if (element.animationEnCours != undefined && element.animationEnCours) {
    		element.nextAction = function(element) {
    			Tapestry.ElementEffect.show(element);
    		}
    		return;	
    	}
    	...
    	if (element.getOpacity() != undefined && element.getOpacity != 1) {
	...
	fade : function(element)
    {
	    ...
	    var options = Object.extend({
	        ...
	        afterFinish: function(effect)
	        {
	            ...
	            effect.element.animationEnCours = false;
	            if (effect.element.nextAction != undefined) {
	            	effect.element.nextAction(effect.element);
	            }
	            effect.element.nextAction = undefined;
	        }
	    }, arguments[1] || { });
...
Tapestry.FormInjector.prototype = {
    ...
    initialize: function(spec)
    {
        ...
        this.element.trigger = function()
        {
			...
			// Ajout de cette ligne pour que Tapestry prenne en compte la surcharge de la fonction "Tapestry.ElementEffect.highlight"
			this.showFunc = Tapestry.ElementEffect[spec.show] || Tapestry.ElementEffect.highlight;
